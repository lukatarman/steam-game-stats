  ...
  describe("run", () => {
    ...
    describe("getxGamesWithoutPlayerHistory returns an empty array and the function finishes", () => {
      beforeAll(() => {
        ...
        const agg = new PlayerHistoryAggregator(steamClientMock, databaseClientMock, {
          batchDelay: 500,
        });
      ...
      it(".getSteamchartsGameHtmlDetailsPage does not run", () => {
        expect(steamClientMock.getSteamchartsGameHtmlDetailsPage).toHaveBeenCalledTimes(
          0,
        );
      });
    });
    ...
    describe("when getxGamesWithoutPlayerHistory returns an array without errors", () => {
      beforeAll(async () => {
        steamClientMock = jasmine.createSpyObj("SteamClient", {
          getSteamchartsGameHtmlDetailsPage: Promise.resolve(
      ...
      it(".getxGamesWithoutPlayerHistory ran before .getSteamchartsGameHtmlDetailsPage", () => {
        expect(databaseClientMock.getxGamesWithoutPlayerHistory).toHaveBeenCalledBefore(
          steamClientMock.getSteamchartsGameHtmlDetailsPage,
        );
      });
      ...
      it(".getSteamchartsGameHtmlDetailsPage has been run with 'tinyGames[0].id' argument", () => {
        expect(steamClientMock.getSteamchartsGameHtmlDetailsPage).toHaveBeenCalledWith(
          tinyGames[0].id,
        );
      });

      it(".getSteamchartsGameHtmlDetailsPage ran before .updatePlayerHistoryById", () => {
        expect(steamClientMock.getSteamchartsGameHtmlDetailsPage).toHaveBeenCalledBefore(
          databaseClientMock.updatePlayerHistoryById,
        );
      });
      ...
      it(".updatePlayerHistoryById has been run with argument tinyGames[1]", () => {
        expect(databaseClientMock.updatePlayerHistoryById).toHaveBeenCalledWith(
          tinyGames[1],
        );
      });
